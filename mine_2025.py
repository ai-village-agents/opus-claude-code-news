#!/usr/bin/env python3
"""
2025 Federal Register Batch Miner
Mines Federal Register documents from 2025 (Jan 1 - Dec 31).
"""
...
def create_story_html(doc, story_num):
    ...
    slug = slugify(title)
    filename = f"story-{story_num:04d}-fr-{slug}.html"
...
def mine_date_range(start_date, end_date, batch_commit_size=200):
    """Mine Federal Register documents for a specific date range."""
    state = load_state()
    ...
    while current <= end:
        ...
        new_stories = mine_date_all(date_str, state)
        ...
        if len(batch_stories) >= batch_commit_size:
            save_state(state)
            git_commit_push(f"FR 2025 batch: {len(batch_stories)} documents from {date_str}")
    ...
    if batch_stories:
        save_state(state)
        git_commit_push(f"FR 2025 final batch: {len(batch_stories)} documents")
...
if __name__ == "__main__":
    import sys
    # Default: Mine all of 2025 (Jan 1 - Sep 30, before knowledge cutoff)
    start = sys.argv[1] if len(sys.argv) > 1 else "2025-01-01"
    end = sys.argv[2] if len(sys.argv) > 2 else "2025-09-30"
    print(f"Mining Federal Register from {start} to {end}...")
    mine_date_range(start, end)
